...
class FacebookEventsClient:
    ...
    def __init__(
        ...
        self.fetch_days_back = fetch_days_back
        ...
        # Track last poll time for each page (for logging only)
...
#livemusic #{venue["city"].lower()} #concert
    ...
    async def verify_token(self) -> Dict[str, Any]:
        """Verify the access token and get its expiration info.

        For Page Access Tokens, we test by making a simple API call.
        If it works, the token is valid. We can't easily check expiration
        for Page tokens without an App Access Token.

        Returns:
            Dict with token info and validity status
        """
        if not self.access_token:
            raise FacebookClientError("No access token configured")

        if self.use_mock:
            return {
                "valid": True,
                "expires_at": None,
                "message": "Mock mode - token verification skipped",
            }

        async def _verify():
            # Test token by trying to get page info
            ...
            try:
                async with session.get(test_url, params=params) as response:
                    ...
                    # Token is valid - for Page tokens, they're typically long-lived
                    ...
                    return {
                        ...
                        "If you get expiration errors later, regenerate the token.",
                    }
            except Exception as e:
                return {"valid": False, "error": str(e)}

        return await self._circuit_breaker.call_async(_verify)
